...
preserve
	...
	sts graph, failure by(group) legend(cols(1)) title("") xtitle(Days from index date) ytitle(Cumulative incidence of death) name(death, replace) plot1opt(lc(black)) plot2opt(lc(black) lp(dash)) plot3opt(lc(gs9) lp(dash_dot))
...
if "`csoutcome'"=="mentalhealth" local outcometext = "Mental health and cognitive (F/G30/X60-84)" 
if "`csoutcome'"=="nervoussystem" local outcometext = "Nervous system (G, ex G30)" 
