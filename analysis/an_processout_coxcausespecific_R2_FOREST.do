...
estimates use analysis/output/models/an_cox_causespecific_R2`outcome'_c`ctrl'_MATCHFACONLY
...
*/
estimates use analysis/output/models/an_cox_causespecific_R2`outcome'_c`ctrl'_COMORBS_LSTYLE_ETHIMD
lincom exposed, eform
...
replace counter = 4 if adjustment=="matchfac" & ctrl=="flu" & outcome=="otherinfections"
...
replace graphorder = 74 if _n==_N
gen one = 1
...
scatter graphorder shr if adjustment=="matchfac", m(Oh) msize(small) mcol(black) || rcap lci uci graphorder if adjustment=="matchfac", hor lw(thin) lc(black) ///
	...
	graph export analysis/output/an_processout_coxcausespecific_R2_FOREST.svg, as(svg) replace
